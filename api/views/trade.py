...
class AlipayTradeView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            # 校验签名
            verify_result = ali_api.pay.trade(processed_dict)
            if not verify_result:
                return HttpResponse("sign is invalid")

            out_trade_no = processed_dict.get("out_trade_no")  # 商户订单号
            redirect_to = "{0}?order_num={1}".format("http://localhost:80/", out_trade_no)
    ...
    def change_order_status(self, order_num, payment_number, gmt_payment, trade_type, extra_params):
        """交易成功修改订单相关的状态
